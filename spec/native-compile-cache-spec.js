...
describe('NativeCompileCache', function() {
  ...
  let [fakeCacheStore, cachedFiles] = [];

  beforeEach(function() {
    cachedFiles = [];
    fakeCacheStore = jasmine.createSpyObj('cache store', [
      'set',
      'get',
      'has',
      'delete'
    ...
    fakeCacheStore.has.andCallFake(
      cacheKey => fakeCacheStore.get(cacheKey) != null
    );
  ...
  describe('when v8 version changes', () =>
    it('updates the cache of previously required files', function() {
  ...
  describe('when a previously required and cached file changes', function() {
    beforeEach(() =>
      fs.writeFileSync(
        path.resolve(__dirname, '/fixtures/native-cache/file-5'),
        `\
...
`
    ...
    it('removes it from the store and re-inserts it with the new cache', function() {
      ...
      fs.appendFileSync(
        require.resolve('./fixtures/native-cache/file-5'),
        '\n\n'
      );
  ...
  it('deletes previously cached code when the cache is an invalid file', function() {
